...
export class LinkedInCollectionExtractor extends BaseJobExtractor {
  ...
  async extract(): Promise<JobDescription> {
    ...
    const { description, usedSelector } = this.extractDescription();
   ...
   */
  private extractPrimaryDescription(): { text: string; selector: string } {
    const selectors = [
   ...
   */
  private extractFitLevelPreferences(): { text: string; selector: string } {
   ...
   */
  private extractMainJobDescription(): { text: string; selector: string } {
    ...
    for (const selector of selectors) {
      ...
      if (element) {
        ...
        if (cleanText && this.isValidJobDescription(cleanText)) {
          return { text: cleanText, selector };
        }
      }
    }
    ...
    if (wrapper) {
      ...
      if (descElement) {
        ...
        if (cleanText && this.isValidJobDescription(cleanText)) {
          return { text: cleanText, selector: "wrapper > description" };
        }
      }
    }
    ...
    if (jobsDetails) {
      ...
      if (descElement) {
        const cleanText = this.extractCleanText(descElement);
        if (cleanText && this.isValidJobDescription(cleanText)) {
          return { text: cleanText, selector: ".jobs-details > description" };
        }
      }
    }
  ...
  private extractDescription(): { description: string; usedSelector: string } {
    const parts: string[] = [];
    ...
    if (combinedDescription) {
      return { description: combinedDescription, usedSelector };
    }

    return { description: "", usedSelector: "" };
  }
}
